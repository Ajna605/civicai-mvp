"""Full vision pipeline: extract images from PDFs and interpret them.
...
def run_vision_pipeline(
    ...
    *,
    ...
    """Extract and interpret all images in a PDF.
    ...
    """
    records = extract_images_from_pdf(pdf_path, min_width=min_width, min_height=min_height)
    if not records:
        return []
    ...
    for rec in records:
        context = (
            f"Page {rec.page}, image {rec.image_index + 1} "
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Extract and interpret images from a PDF using the OpenAI Vision API."
    )
    p.add_argument("--source", required=True, help="Path to the input PDF file.")
    p.add_argument(
        "--out",
        default=None,
        help=(
            "Output JSONL path for image description chunks. "
            "Defaults to data/normalized/pdf/image_descriptions.jsonl."
        ),
    )
    p.add_argument(
        "--model",
...
def main() -> None:
    ...
    out_path = args.out or str(
        Path("data/normalized/pdf/image_descriptions.jsonl")
    )

    api_key = os.environ.get("OPENAI_API_KEY")

    print(f"[vision.pipeline] Source PDF : {args.source}")
