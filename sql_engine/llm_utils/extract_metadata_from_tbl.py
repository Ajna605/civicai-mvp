...
def parse_args():
    ...
    p.add_argument("--table", default="facts")
    return p.parse_args()
...
def build_measure_groups(con: duckdb.DuckDBPyConnection, table: str):
    ...
    rows = con.execute(f"""
        SELECT
          source_file,
          orig_row_id,
          ANY_VALUE(raw_row) AS raw_row_text,
    ...
    for source_file, orig_row_id, raw_row_text, measure, has_numeric_value in rows:
        m = (measure or "").strip()
        if not m:
            continue
    ...
    return groups, measure_to_group, headings, leaf_measures
...
def export_metadata(con: duckdb.DuckDBPyConnection, table="facts") -> dict:
    ...
    measure_groups, measure_to_group, measure_headings, leaf_measures = build_measure_groups(con, table)
